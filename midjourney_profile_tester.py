...
if not st.session_state.fullscreen:
    ...
    if profile_results_dir.exists():
        existing_profiles = [d.name for d in profile_results_dir.iterdir() if d.is_dir() and d.name != 'baseline']
            ...
            for row_idx in range(0, len(all_images), 5):
                ...
                for col_idx, col in enumerate(cols):
                    ...
                    if img_idx < len(all_images):
                        ...
                        with col:
                            st.image(str(filepath), caption=test_name, use_container_width=True)
                        ...
                        if test_idx < len(tests_list):
                            ...
                            with col:
                                ...
                                if existing_filepath:
                                    st.image(str(existing_filepath), use_container_width=True)
...
Be thorough and specific in your analysis."""
                ...
                try:
                    ...
                    with st.spinner("🔍 Finding matching profiles..."):
                        ...
                        if profile_analyses_dir.exists():
                            for analysis_file in profile_analyses_dir.glob("*_analysis.json"):
                                profile_id = analysis_file.stem.replace("_analysis", "")
                                try:
                                    import json
                                    with open(analysis_file, 'r') as f:
                                        data = json.load(f)
...
5. Get ready-to-use prompts with the best matching profiles""")
    ...
    analyses = {}
    json_files = list(profile_analyses_dir.glob("*_analysis.json"))
        ...
        for json_file in json_files:
            try:
                with open(json_file, 'r') as f:
                    data = json.load(f)
                    profile_id = data.get('profile_id', json_file.stem.replace('_analysis', ''))
                    analyses[profile_id] = data
            except Exception as e:
                ...
                if profile_analyses_dir.exists():
                    ...
                    for analysis_file in profile_analyses_dir.glob("*_analysis.json"):
                        try:
                            with open(analysis_file, 'r') as f:
                                data = json.load(f)
                                profile_id = data.get('profile_id', analysis_file.stem.replace('_analysis', ''))
                    ...
                    st.info("No profile analyses directory found")
