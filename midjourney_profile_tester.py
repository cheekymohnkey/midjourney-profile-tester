...
def find_image_file(output_dir, profile_id, test_name):
    """
    ...
    jpg_path = output_dir / f"{profile_id}_{safe_name}.jpg"
    if storage.exists(str(jpg_path)):
        return jpg_path
    
    # Fall back to .png (legacy format)
    png_path = output_dir / f"{profile_id}_{safe_name}.png"
...
**Your Task**: For each test image, evaluate how well the image ACHIEVES THE AESTHETIC DESCRIBED IN THE TEST PROMPT.
...
- **affinity**: One of ["native_fit", "workable", "resistant"] - Use "resistant" liberally when the prompt is compromised
  * native_fit (scores 8-10): The prompt's aesthetic was executed excellently with no significant compromises
  * workable (scores 5-7): The prompt's aesthetic was partially achieved but with notable compromises, style drift, or technical mismatches
  * resistant (scores 1-4): The prompt's aesthetic was poorly executed, wrong medium/mood/subject, or fundamentally missed the mark
- **score**: Integer 1-10 rating of how well the prompt was fulfilled
- **confidence**: Float 0.0-1.0 indicating your confidence in the rating
- **commentary**: 3-4 sentences explaining: (1) How well the image achieves the prompt's requested aesthetic (medium accuracy, mood match, subject correctness), AND (2) The profile's unique aesthetic signature in this image - describe tonal qualities (dark/light, warm/cool), color palette (dominant colors, saturation levels), texture qualities (smooth/gritty, painterly/photographic), and any distinctive stylistic choices that characterize this profile's interpretation.
- **color-palette**: 1-2 sentences explaining the color palette found in the image, cover dominant, secondary and accent colors, saturation and luminance, hue and temperature, use Descriptive Adjectives where appropriate: Instead of just "blue," use "azure," "navy," or "baby blue"
...
**Test Images:**"""
    ...
    for idx, (test_name, filepath, row) in enumerate(batch_tests, 1):
                ...
                with st.spinner("Analyzing prompt..."):
                    ...
                    try:
                        ...
                        for rank, (profile_id, info) in enumerate(sorted_profiles[:5], 1):
                            ...
                            with st.expander(f"{medal} **#{rank}: {profile_id}** (Score: {score:.1f}/10)", expanded=(rank <= 3)):
                                # Show Profile DNA
                                st.markdown("**Profile DNA:**")
                                dna_traits = data.get('profile_dna', [])
                                if dna_traits:
